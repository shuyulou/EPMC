...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        self.trans_ttchange = TransformerEncoder(embed_dim=embed_dim,
                                                 ...
                                                 embed_dropout=embed_dropout,
                                                 attn_mask=attn_mask)
        self.ii_proj = nn.Linear(embed_dim, embed_dim) #全连接
        self.tt_proj = nn.Linear(embed_dim, embed_dim)    

        # create momentum models
        self.visual_encoder_m = VisionTransformer(
            img_size=config['image_res'], patch_size=16, embed_dim=768, depth=12, num_heads=12, 
            mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6)) #与图像 文本 编码器一致
        ...
        self.trans_ttchange_m = TransformerEncoder(embed_dim=embed_dim,
                                                 ...
                                                 embed_dropout=embed_dropout,
                                                 attn_mask=attn_mask)
        self.ii_proj_m = nn.Linear(embed_dim, embed_dim) #全连接
        ...
        self.trans_tichange = TransformerEncoder(embed_dim=embed_dim,
                                                 ...
                                                 embed_dropout=embed_dropout,
                                                 attn_mask=attn_mask)
        self.it_proj = nn.Linear(embed_dim, embed_dim) #全连接
    ...
    def forward(self, image, text, alpha=0):
        ...
        ## image_feat_old = F.normalize(self.vision_proj(image_embeds_old[:,0,:]),dim=-1)  #归一化
        # print("image_feat={}".format(np.shape(image_embeds_old)))
        ...
        ## text_feat_old = F.normalize(self.text_proj(text_embeds_old[:,0,:]),dim=-1) #归一化   
        # print("text_feat={}".format(np.shape(text_embeds_old))) 
        # print("text.attention_mask={}".format(np.shape(text.attention_mask)))  
        ...
        with torch.no_grad():
            ...
            image_embeds_m_old = self.visual_encoder_m(image) #图像动量编码
            text_output_m = self.text_encoder_m.bert(text.input_ids, attention_mask = text.attention_mask,                      
                                                return_dict = True, mode = 'text') #文本动量编码
            ...
            sim_targets.fill_diagonal_(1) #对角线填充1         

            sim_i2t_targets = alpha * F.softmax(sim_i2t_m, dim=1) + (1 - alpha) * sim_targets #softmax
        ...
        loss_i2t = -torch.sum(F.log_softmax(sim_i2t, dim=1)*sim_i2t_targets,dim=1).mean() #原模型与动量模型的KL散度
        ...
        # loss_t2i_c = -torch.sum(F.log_softmax(sim_t2i_c, dim=1)*sim_targets,dim=1).mean()
        ...
        # text_embeds = text_embeds.view(text_embeds.size()[1], text_embeds.size()[0], text_embeds.size()[2])
        # image_embeds = image_embeds.view(image_embeds.size()[1], image_embeds.size()[0], image_embeds.size()[2])
        ...
        # print("text_embeds={}".format(np.shape(text_embeds)))
        # print("text_atts={}".format(np.shape(text_atts)))
        # print("image_embeds={}".format(np.shape(image_embeds)))
        # print("image_atts={}".format(np.shape(image_atts)))
        ...
        # output_pos = self.trans_itchange(text_embeds, )
        ...
        with torch.no_grad():
            ...
            # print("weights_i2t={}".format(np.shape(weights_i2t)))   
   
            weights_i2t.fill_diagonal_(0) #对角置0 保证不会抽到与自己相匹配的数据
            weights_t2i.fill_diagonal_(0)

        # select a negative image for each text
        image_embeds_neg = []    
        for b in range(bs):
            neg_idx = torch.multinomial(weights_t2i[b], 1).item() #随机抽一个
        ...
        # print("text_embeds_all={}".format(np.shape(text_embeds_all)))
        # print("text_atts_all={}".format(np.shape(text_atts_all)))
        # print("image_embeds_all={}".format(np.shape(image_embeds_all)))
        # print("image_atts_all={}".format(np.shape(image_atts_all)))
        ...
        with torch.no_grad():
            ...
            # image_embeds_m = self.trans_itchange(m_text_feat, m_image_feat, m_image_feat)
        ...
        if np.isnan(loss_mlm.item()):
            print("logits_m={}".format(logits_m))
    ...
    def _momentum_update(self):
        for model_pair in self.model_pairs:           
            for param, param_m in zip(model_pair[0].parameters(), model_pair[1].parameters()):
                param_m.data = param_m.data * self.momentum + param.data * (1. - self.momentum) #动量更新
