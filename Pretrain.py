...
from models.model_pretrain_m_9_6 import ALBEF
...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    for i, (image, text) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_mlm, loss_ita, loss_itc, loss_itm = model(image, text_input, alpha = alpha)  
            
        loss = loss_mlm + loss_ita + loss_itc + loss_itm    

        # loss_mlm, loss_itm = model(image, text_input, alpha = alpha)  
        ...
        if np.isnan(loss_mlm.item()):
            metric_logger.update(loss_mlm=1)
            print("/n/nlou/n/n")
        else:
...
def main(args, config):
    ...
    # fusion_tokenizer = BertTokenizer.from_pretrained(args.fusion_encoder)

    #### Model #### 
    print("Creating model")
    model = ALBEF(config=config, text_encoder=args.text_encoder, tokenizer=tokenizer, fusion_encoder=args.fusion_encoder, init_deit=True)
    # model = ALBEF(config=config, text_encoder=args.text_encoder, tokenizer=tokenizer, init_deit=True)

    model = model.to(device)   

    #过滤参数
    # freeze_layers = ['text_encoder.cls']
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--fusion_encoder', default='bert-base-uncased')
    # parser.add_argument('--text_encoder', default='gaunernst/bert-mini-uncased')
