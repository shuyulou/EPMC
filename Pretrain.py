...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    metric_logger.add_meter('loss_itd', utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
    ...
    for i, (image, text) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_mlm, loss_itd, loss_itc, loss_itm = model(image, text_input, alpha = alpha)  
            
        loss = loss_mlm + loss_itd + loss_itc + loss_itm    
        ...
        metric_logger.update(loss_itd=loss_itd.item())
