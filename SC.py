...
from models.model_pretrain_ev_0411 import ALBEF
...
def evaluate(model, data_loader, tokenizer, device, config, output_dir, flag=0):
    ...
    for image, text, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # prediction_vector = model(image, text_input)
...
def main(args, config):
    ...
    datasets = create_dataset('sc', config)
    ...
    model = ALBEF(config=config, text_encoder=args.text_encoder, tokenizer=tokenizer, fusion_encoder=args.fusion_encoder)
    ...
    if args.checkpoint:    
        ...
        #     print(key)
        # state_dict = checkpoint['model']
        # print(state_dict['text_encoder.pos_embed'])
        # print(state_dict)       
